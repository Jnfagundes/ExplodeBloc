...
container.addEventListener('click', (e) => {
  ...
  if (!blocoSelecionado) {
    blocoSelecionado = { i, j };
    alvo.classList.add('selecionado');
  } else {
    const { i: i2, j: j2 } = blocoSelecionado;
...
function trocarBlocos(i1, j1, i2, j2) {
  ...
  const cor1 = grade[i1][j1];
  const cor2 = grade[i2][j2];
